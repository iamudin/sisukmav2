  ...
  <form class="" action="{{$edit->exists ? route('unsur.update', $edit->id) : route('unsur.store') }}" method="post">
    ...
    <div class="form-group mb-4">
      ...
      <div class="mb-2">
        ...
        <select name="kategori_unsur_id" id="" class="form-control">
          <option value="">--pilih--</option>
          @foreach(Sisukma\V2\Models\KategoriUnsur::get() as $kategori)
            <option value="{{ $kategori->id }}" {{ $edit->KategoriUnsur && $kategori->id==$edit->kategoriUnsur->id ? 'selected':null }}> {{ $kategori->nama }} </option>
