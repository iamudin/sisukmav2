...
<tr>
    ...
    <td align="center">*)
        {{ round((($u1 / $respon) ) * 1/ 9 + (($u9 / $respon)  ) * 1/ 9 + (($u2 / $respon) * 1) / 9 + (($u3 / $respon) * 1) / 9 + (($u4 / $respon) * 1) / 9 + (($u5 / $respon) * 1) / 9 + (($u6 / $respon) * 1) / 9 + (($u7 / $respon) * 1) / 9 + (($u8 / $respon) * 1) / 9, 2) }}
    </td>
</tr>
...
<tr>
    ...
    <td align="center">**) <b>{{round(((($u1 / $respon * (1/9))*1/9)+(($u9 / $respon * (1/9))*1/9)+(($u2 / $respon * (1/9))*1/9)+(($u3 / $respon * (1/9))*1/9)+(($u4 / $respon * (1/9))*1/9)+(($u5 / $respon * (1/9))*1/9)+(($u6 / $respon * (1/9))*1/9)+(($u7 / $respon * (1/9))*1/9)+(($u8 / $respon * (1/9))*1/9))*25,2)}}</b></td>
</tr>
@else
<tr>
...
<td style="border:none" rowspan="12">
    ...
    <h5 style="border:2px solid #000;padding:10px;margin-right:50px">IKM UNIT PELAYANAN <span
            style="float:right">{{round(((($u1 / $respon)*1/9)+(($u9 / $respon)*1/9)+(($u2 / $respon)*1/9)+(($u3 / $respon)*1/9)+(($u4 / $respon)*1/9)+(($u5 / $respon)*1/9)+(($u6 / $respon)*1/9)+(($u7 / $respon)*1/9)+(($u8 / $respon)*1/9))*25,2)}}</span></h5>
...
<tr>
    ...
    <td style="border:1px solid #000;padding-left:4px">
        {{ [1 => 'Persyaratan', 'Prosedur', 'Waktu Pelayanan', 'Biaya / Tarif', 'Produk Pelayanan', 'Kompetensi Pelaksana', 'Perilaku Pelaksana', 'Penanganan Pengaduan Saran dan Masukan', 'Sarana dan Prasarana'][$i] }}
    </td>
    <td style="border:1px solid #000;padding-right:20px;text-align:right">
        {{ [1 => round($u1 / $respon, 2), round($u2 / $respon, 2), round($u3 / $respon, 2), round($u4 / $respon, 2), round($u5 / $respon, 2), round($u6 / $respon, 2), round($u7 / $respon, 2), round($u8 / $respon, 2), round($u9 / $respon, 2)][$i] }}
    </td>
