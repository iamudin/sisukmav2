...
<div class="container-fluid px-4">
  ...
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">{{$layanan->nama_skpd}}</li>
  </ol>
</nav>
    <a href="{{ url('admin/layanan') }}" style="float:right" class="btn btn-danger">X</a>
    @if(count($resp)>0)
    <a href="{{request('tahun') || request('bulan') ? URL::full().'&cetak=true' : URL::full().'?cetak=true'}}" class="btn btn-sm btn-primary"> <i class="fa fa-print"></i> Cetak</a> 
    <select onchange="if(this.value)location.href=this.value" id="">
      <option value="">--pilih tahun--</option>
      @foreach(getYear($layanan->id_layanan) as $r)
      <option {{request('tahun') && request('tahun')==$r? 'selected':''}} value="{{url('admin/respon-layanan/'.$id.'?tahun='.$r)}}">{{$r}}</option>
      @endforeach
    </select>
    @if(request('tahun'))
    <select onchange="if(this.value)location.href=this.value" id="">
      ...
      <option {{request('bulan') && request('bulan')==$r? 'selected':''}} value="{{url('admin/respon-layanan/'.$id.'?tahun='.request('tahun').'&bulan='.$r)}}">{{blnindo($r)}}</option>
      @endforeach
    </select>
    @endif
    <br><br>
...
<table border="1" style="width:100%;border-style:solid;border-color:#000;font-size:10px" class="table table-bordered">
    <tr>
