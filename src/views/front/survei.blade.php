...
@section('content')
    ...
    <!-- STATUS DISABILITAS -->
    <div class="row q-all disabilitas" style="display:none">
      <center>
        <h1>Kategorisasi Pengguna Layanan</h1>
      </center>
      <br><br><br>

      <div class="col-lg-12 col-12">
        <input type="hidden" name="disabilitas">

        <div class="list-group">
          @foreach(['Non Disabilitas', 'Disabilitas'] as $k => $r)
            <button type="button" onclick="
                  $('input[name=disabilitas]').val('{{$r}}');
                  $('.disab').removeClass('active');
                  $('.disab-{{$k}}').addClass('active');
                  if('{{$r}}'=='Disabilitas'){
                    $('.next-jenis-disabilitas').show();
                    $('.next-emot').hide();
                  }else{
                    $('.next-emot').show();
                    $('.next-jenis-disabilitas').hide();
                  }
                " class='disab disab-{{$k}} w-100 list-group-item list-group-item-action'>
              <b>{{$r}}</b>
            </button>
          @endforeach
        </div>
        ...
        <div>
          <button type="button" class="btn btn-danger btn-md float-start previous" style="width:150px"
            onclick="$('.q-all').hide();$('.kerja').show()">
            <i class="fa fa-angle-left" aria-hidden="true"></i> Sebelumnya
          </button>

          <button type="button" class="btn btn-primary btn-md float-end next-jenis-disabilitas"
            onclick="$('.q-all').hide();$('.jenis-disabilitas').show()" style="width:150px;display:none">
            Lanjutkan <i class="fa fa-angle-right" aria-hidden="true"></i>
          </button>

          <button type="button" class="btn btn-primary btn-md float-end next-emot"
            onclick="$('.q-all').hide();$('.q-1').show()" style="width:150px;display:none">
            Lanjutkan <i class="fa fa-angle-right" aria-hidden="true"></i>
          </button>
        </div>
      </div>
    ...
    <!-- JENIS DISABILITAS -->
    <div class="row q-all jenis-disabilitas" style="display:none">
      <center>
      ...
      <div class="col-lg-12 col-12">
        <input type="hidden" name="jenis_disabilitas">

        <div class="list-group">
          @foreach(['Disabilitas Fisik', 'Disabilitas Intelektual', 'Disabilitas Mental', 'Disabilitas Sensorik'] as $k => $r)
            <button type="button" onclick="
                  $('input[name=jenis_disabilitas]').val('{{$r}}');
                  $('.jd').removeClass('active');
                  $('.jd-{{$k}}').addClass('active');
                  $('.next-emot2').show();
                " class='jd jd-{{$k}} w-100 list-group-item list-group-item-action'>
              <b>{{$r}}</b>
            </button>
          @endforeach
        </div>
        ...
        <div>
          <button type="button" class="btn btn-danger btn-md float-start previous" style="width:150px"
            onclick="$('.q-all').hide();$('.disabilitas').show()">
            <i class="fa fa-angle-left" aria-hidden="true"></i> Sebelumnya
          </button>

          <button type="button" class="btn btn-primary btn-md float-end next-emot2"
            onclick="$('.q-all').hide();$('.q-1').show()" style="width:150px;display:none">
            Lanjutkan <i class="fa fa-angle-right" aria-hidden="true"></i>
          </button>
        </div>
